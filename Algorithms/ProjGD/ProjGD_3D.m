function [si,iter,x,ratio,fv,gm,step,t] = ... 
    ProjGD_3D(obs,n1,n2,n3,r,K,maxit,tol_1,tol_2,opt_1,opt_2,trace)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Projected gradient descent for recovering 3D spectrally sparse signals
% via low rank 3-level Hankel matrix completion.
%
%Inputs
...
%    n3: length of the signal to be recovered in the third dimension.
...
l1 = p1*p2*p3; % number of rows of the 3-level Hankel matrix
l2 = q1*q2*q3; % number of columns of the 3-level Hankel matrix
...
L3 = 2^nextpow2(n3); % next power of 2 for faster fft
...
for iter = 1:maxit
    ...
    y1 = zeros(n1,n2,n3);
    for i = 1:r
        vi = reshape(conj(Vg(:,i)),q1,q2,q3);
        ui = reshape(fft_U(:,i),LL{1},LL{2},LL{3});
        for dim = 1:3
            vi = fft(vi,LL{dim},dim);
        end      
    ...
    y2 = zeros(n1,n2,n3);
    for i = 1:r
        ui = reshape(Ug(:,i),p1,p2,p3);
        for dim = 1:3
            ui = fft(ui,LL{dim},dim);
        end
        vi = reshape(fft_V(:,i),LL{1},LL{2},LL{3});
    ...
    y3 = zeros(n1,n2,n3);
    for i = 1:r
        ui = reshape(fft_Ug(:,i),LL{1},LL{2},LL{3});
        vi = reshape(fft_Vg(:,i),LL{1},LL{2},LL{3});
        ui = ui.*vi;
        for dim = 1:3
            ui = ifft(ui,[],dim);
        end
...
temp = y(K)-nobs;
value_f = sum(sum(conj(UtU).*VtV))-sum(sum(sum(conj(y).*y)))+alpha*(temp'*temp);
TEMP = UtU-VtV;
