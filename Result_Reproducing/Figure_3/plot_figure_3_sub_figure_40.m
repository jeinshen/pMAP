function plot_figure_3_sub_figure_40(solver_choice, recovered_series)
...
tau = 1;
M = 249;
N = length(recovered_series(:, 5));
r = 40;
figure_font_size = 20;

ox1 = recovered_series(:, 5);
x_mpm = recovered_series(:, solver_choice + 5);
...
ak=real(pinv(exp(-1i*2*pi/tau*(-M:M)'*tk'))*ox1);
    ...
    f = figure('Name',sprintf('Solver %s, rank = 40', solver), 'NumberTitle','off', 'visible','off');
...
Hp1 = hankel(x_mpm(1:(N-r)), x_mpm((N-r):end));

[~, ~, V2] = svd(Hp1, 0);
estimtk=-angle(roots(flipud(V2(:,r+1))))*tau/2/pi;	%estimated locations in [-tau/2,tau/2[
estimtk = sort(estimtk);

ind=find(estimtk<0);
estimtk(ind)=estimtk(ind)+tau;			%estimated locations in [0,tau[
estimak=real(pinv(exp(-1i*2*pi/tau*(-M:M)'*estimtk'))*ox1);	%estimated amplitudes
edgecolor='r';
...
if solver_choice == 1
    out_put_file_name = 'Output/Fig_3(b).eps';
elseif solver_choice == 2
    out_put_file_name = 'Output/Fig_3(d).eps';
else
    out_put_file_name = 'Output/Fig_3(f).eps';
end
